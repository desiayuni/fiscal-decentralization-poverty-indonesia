	...
	** Log and macro
		* please specify the folder for the data, log-file and outreg. Note: the outreg folder should be in that specific location. 
		global project "D:\Documents\Internship\VSLA-Replication-Project"
global data "${project}\data"
global dofiles "${project}\dofiles"
global outputs "${project}\outputs"
global graphs "${project}\graphs"
global paper "${project}\paper"
global reports "${project}\reports"
	
	capture log close
		log using "${outputs}\main_output_log.smcl", replace

	** Load data	
		use "${data}\impact_replication_final.dta", clear
